...
- La aceptacion de terminos y tratamiento de datos para clientes publicos de Agenda debe ser versionada, guardada en servidor y asociada al `customerUid`. No pedirla en cada cita si la version vigente ya fue aceptada.
- Si el texto legal de Agenda/Reservas cambia, actualizar una version legal propia de Agenda y no reutilizar automaticamente la version general del panel, para no interrumpir a duenos de negocios sin necesidad.
