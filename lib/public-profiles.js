...
export function buildPublicProfileDescription(user = {}) {
  ...
  const labels = (user.profileLinks || [])
    .map((item) => String(item.label || "").trim())
    .filter(Boolean)
    .slice(0, 2);

  if (labels.length >= 2) {
    return `Encuentra ${labels[0]}, ${labels[1]} y mas enlaces de ${businessName} en un solo lugar.`;
  }

  if (labels.length === 1) {
    return `Encuentra ${labels[0]} y la informacion de ${businessName} en un solo lugar.`;
  }

  return `Descubre la pagina publica de ${businessName} y conecta con su negocio en segundos.`;
}
