...
export function normalizeBookingConfig(raw = {}) {
  ...
  return {
    ...
    noticeMinutes: clampNumber(source.noticeMinutes ?? source.bookingNoticeMinutes, 0, BOOKING_NOTICE_MINUTES_LIMIT, 0),
    maxDaysAhead: clampNumber(source.maxDaysAhead ?? source.bookingMaxDaysAhead, 1, BOOKING_MAX_DAYS_AHEAD_LIMIT, 30),
    notifyBusinessOnRequest: source.notifyBusinessOnRequest !== false,
    notifyCustomerOnConfirmation: source.notifyCustomerOnConfirmation !== false,
    reminderEnabled: Boolean(source.reminderEnabled ?? false),
    reminderMinutesBefore: clampNumber(source.reminderMinutesBefore, 15, BOOKING_NOTICE_MINUTES_LIMIT, 60),
    reactivationEnabled: Boolean(source.reactivationEnabled ?? false),
    reactivationDays: clampNumber(source.reactivationDays, 7, 180, 30),
