...
import { ACCOUNT_STATES, DEFAULT_THEME, GRACE_DAYS, HARD_SUSPENSION_DAYS, PLAN_SLUG, RESERVED_USERNAMES, TRIAL_DAYS } from "@/lib/constants";
import { getAdminDb, getAdminStorage } from "@/lib/firebase-admin";
import { buildWhatsappLink, getCancellationDate, getGraceEnd, getTrialEnd, getUsernameChangeDate, normalizeUrl, sanitizePhone, sanitizeSlug, toDate } from "@/lib/utils";
...
export async function ensureUserProfile({ uid, email, name = "", photoURL = "", role = "user" }) {
  ...
  const data = {
    ...
    trialEndsAt: Timestamp.fromDate(getTrialEnd(now)),
    expiresAt: null,
    graceUntil: null,
    cancellationAt: null,
...
export async function updateUserProfile(uid, input, file) {
  ...
  if (usernameChanged) {
    ...
    if (!available) {
      throw new Error("Ese usuario ya esta en uso o no esta permitido");
    }
    const nextChangeAllowed = toDate(current.usernameChangeAvailableAt);
    if (nextChangeAllowed && new Date() < nextChangeAllowed) {
      throw new Error("Solo puedes cambiar el usuario una vez cada 30 dias");
    }
  }
  ...
  if (usernameChanged) {
    ...
    payload.usernameChangeAvailableAt = Timestamp.fromDate(getUsernameChangeDate(new Date()));
  }
...
export function getAccountView(user) {
  ...
  return {
    ...
    graceUntil,
    cancellationAt,
    whatsappUrl: buildWhatsappLink(user.links?.whatsapp || ""),
  };
}
...
export async function activateUserSubscription(uid, paymentData) {
  ...
  await paymentsCollection().doc(paymentData.id).set({
    ...paymentData,
    uid,
    createdAt: FieldValue.serverTimestamp(),
  });
