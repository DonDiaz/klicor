...
import { buildCommercePublicUrl, normalizeCommerceConfig, resolveDefaultCommerceModeForBusinessCategory } from "@/lib/commerce-config";
...
const COMMERCIAL_INTENT_ACTIONS = new Set(["product_whatsapp", "detail_whatsapp", "cart_whatsapp"]);
...
export async function recordUserLegalAcceptance(uid, input = {}, metadata = {}) {
  ...
  if (!acceptance.accepted) {
    throw new Error("Debes aceptar los términos y condiciones para crear o usar tu cuenta.");
  }
...
export async function uploadProfilePhoto(uid, file) {
  ...
  const bucket = getAdminStorage();
  const imageVersion = `img_${crypto.randomBytes(6).toString("hex")}`;
  const basePath = `profiles/${uid}/${imageVersion}`;
  ...
  await Promise.allSettled([
    bucket.file(`profiles/${uid}/cover.jpg`).delete({ ignoreNotFound: true }),
    bucket.file(`profiles/${uid}/cover.png`).delete({ ignoreNotFound: true }),
    bucket.file(`profiles/${uid}/cover.webp`).delete({ ignoreNotFound: true }),
  ]);
  ...
  return {
    ...
    thumbPath,
    thumbUrl: buildPublicStorageUrl(bucket.name, thumbPath),
...
function invalidatePublicCommerceCacheForUser(user = {}) {
  ...
  if (slug && mode) {
    revalidateTag(`public-commerce:${slug}:${mode}`);
    revalidatePath(buildCommercePublicUrl(slug, mode));
  }
}

function invalidatePublicCommerceCacheForMode(username = "", mode = "") {
  const slug = sanitizeSlug(username);
  const cleanMode = normalizeCommerceConfig({ activeMode: mode }, { commerceMode: mode }).activeMode;
  if (slug) revalidateTag(`public-commerce:${slug}`);
  if (slug && cleanMode) {
    revalidateTag(`public-commerce:${slug}:${cleanMode}`);
    revalidatePath(buildCommercePublicUrl(slug, cleanMode));
  }
}
...
export async function updateUserProfile(uid, input, assets = {}) {
  ...
  if (assets.photo) {
    ...
    await removeStoragePaths([
      current.photoPath !== nextUser.photoPath ? current.photoPath : "",
      current.photoThumbPath !== nextUser.photoThumbPath ? current.photoThumbPath : "",
    ]);
  }
  ...
  if (nextUser.usernameLower) {
    revalidateTag(`public-booking:${nextUser.usernameLower}`);
    revalidatePath(`/${nextUser.usernameLower}/agenda`);
  }
  if (currentSlug && currentSlug !== nextUser.usernameLower) {
    revalidateTag(`public-booking:${currentSlug}`);
    revalidatePath(`/${currentSlug}/agenda`);
  }
  if (shouldSwitchCommerceMode) {
...
export async function enableUserModule(uid, module) {
  ...
  if (plan === "commercial") {
    throw new Error("En Emprendedor solo puedes usar un módulo. Actualiza a Comercial Plus para usar Comercio y Agenda.");
  }
...
export async function trackCommercialIntent(user = {}, { action = "", mode = "", productId = "", productName = "" } = {}) {
  const uid = String(user.uid || "").trim();
  const slug = sanitizeSlug(user.usernameLower || user.username || "");
  const metricKey = normalizeAnalyticsKey(action);
  if (!uid || !slug || !COMMERCIAL_INTENT_ACTIONS.has(metricKey)) return;

  const monthKey = getCommercialUsageMonthKey();
  const docId = `${uid}_${monthKey}`;
  const actionField = `action_${metricKey}`;

  await commercialUsageCollection().doc(docId).set({
    uid,
    username: slug,
    businessName: String(user.businessName || "").trim(),
    plan: normalizePlan(user.plan),
    mode: normalizeAnalyticsKey(mode || user.commercialModule || user.module || "commerce"),
    monthKey,
    total: FieldValue.increment(1),
    [actionField]: FieldValue.increment(1),
    lastAction: metricKey,
    lastProductId: String(productId || "").trim().slice(0, 120),
    lastProductName: String(productName || "").trim().slice(0, 180),
    updatedAt: FieldValue.serverTimestamp(),
  }, { merge: true });
}
