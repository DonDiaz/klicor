...
import { DEFAULT_BUSINESS_CATEGORY, normalizeBusinessCategory, normalizeBusinessType } from "@/lib/business-categories";
...
export async function updateUserProfile(uid, input, assets = {}) {
  ...
  if (businessCategoryChanged && shouldRestrictToPrimaryModuleOnProfileChange(current)) {
    Object.assign(payload, buildModuleAccessPayload({ ...current, businessCategory }, { plan: current.plan || current.status }));
  } else if (current.status === ACCOUNT_STATES.TRIAL && current.plan === "trial" && !current.moduleAccess) {
    Object.assign(payload, buildModuleAccessPayload({ ...current, businessCategory }, { plan: "trial" }));
  }
  ...
  if (shouldSwitchCommerceMode) {
    invalidatePublicCommerceCache({
      currentUsername: nextUser.usernameLower,
      mode: currentCommerceConfig.activeMode,
    });
  }
  return nextUser;
}
