...
export async function generateAndStoreStableQr(publicLinkId) {
  ...
  const pngBuffer = await generateBrandedQrBuffer(url, { size: 400 });
...
export async function updateUserProfile(uid, input, assets = {}) {
  ...
  const paymentMethods = normalizePaymentMethods(input.paymentMethods, currentProfileLinks, current.paymentQrUrl);
  const currentPaymentMethods = normalizePaymentMethods(current.paymentMethods, currentProfileLinks, current.paymentQrUrl);
  ...
  } else if (assets.removePaymentQr || !hasConfiguredPaymentMethods || paymentMethodsChanged) {
...
export function getAccountView(user) {
  ...
  const paymentMethods = normalizePaymentMethods(user.paymentMethods, profileLinks, user.paymentQrUrl);
