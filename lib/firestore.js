...
export async function activateUserSubscription(uid, paymentData) {
  ...
  const now = new Date();
  const baseDate = current?.expiresAt ? toDate(current.expiresAt) : null;
  const renewalBase = baseDate && baseDate > now ? new Date(baseDate) : new Date(now);
  ...
  await storePaymentAttempt(uid, paymentData);
}
