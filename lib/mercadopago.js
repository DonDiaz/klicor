import "server-only";
import { getAppUrl } from "@/lib/env";
import { getPlanDefinition, normalizeKlicorPlan } from "@/lib/plans";

async function mpFetch(path, init = {}) {
  if (!process.env.MERCADOPAGO_ACCESS_TOKEN) {
    throw new Error("Falta MERCADOPAGO_ACCESS_TOKEN");
  }

  const response = await fetch(`https://api.mercadopago.com${path}`, {
    ...init,
    headers: {
      Authorization: `Bearer ${process.env.MERCADOPAGO_ACCESS_TOKEN}`,
      "Content-Type": "application/json",
      ...(init.headers || {}),
    },
    cache: "no-store",
  });

  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Mercado Pago error: ${body}`);
  }

  return response.json();
}

export async function createPreference({ user, plan = "commercial", annualPrice, metadata = {}, titleSuffix = "pago anual" }) {
  const appUrl = getAppUrl();
  const normalizedPlan = normalizeKlicorPlan(plan);
  const planDefinition = getPlanDefinition(normalizedPlan);

  return mpFetch("/checkout/preferences", {
    method: "POST",
    body: JSON.stringify({
      items: [
        {
          title: `Plan ${planDefinition.publicName || planDefinition.label} Klicor - ${titleSuffix}`,
          quantity: 1,
          currency_id: "COP",
          unit_price: annualPrice,
        },
      ],
      external_reference: user.uid,
      metadata: {
        uid: user.uid,
        username: user.usernameLower || "",
        plan: normalizedPlan,
        ...metadata,
      },
      notification_url: `${appUrl}/api/billing/webhook`,
      back_urls: {
        success: `${appUrl}/billing/success`,
        pending: `${appUrl}/billing/success?status=pending`,
        failure: `${appUrl}/billing/success?status=failure`,
      },
      auto_return: "approved",
    }),
  });
}

export async function getPayment(paymentId) {
  return mpFetch(`/v1/payments/${paymentId}`);
}
