...
const ACTION_BLUEPRINTS = {
  food_drink: ["whatsapp", "website", "maps"],
  retail_sales: ["whatsapp", "marketplace", "maps"],
  services: ["whatsapp", "website", "maps"],
  health_wellness: ["whatsapp", "website", "maps"],
...
export function getOnboardingActionBlueprints(category) {
  ...
  return types.map((type, index) => ({
    ...
    type,
    label: template.suggestedLabels[type] || LINK_CATALOG_MAP[type]?.label || "Accion",
...
export function buildOnboardingProfileLinks(state, profile = {}) {
  ...
  const onboardingLinks = state.actionSlots
    ...
    .map((slot) => ({
      id: slot.id,
      type: slot.type,
      label: String(slot.label || LINK_CATALOG_MAP[slot.type]?.label || "Accion").trim(),
      value: String(slot.value || "").trim(),
      message: slot.type === "whatsapp" ? String(slot.message || "Hola, quiero informacion").trim() : "",
    }));
