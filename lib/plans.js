import { isBusinessModuleEligible, isBookingEligibleBusiness, isCommerceEligibleBusiness } from "@/lib/business-categories";
...
export const KLICOR_PLAN_DEFINITIONS = {
  ...
  plus: {
    value: "plus",
    label: "Comercial Plus",
    publicName: "Comercial Plus",
    annualPrice: 169900,
  ...
  pro: {
    value: "pro",
    label: "Business 500",
    publicName: "Business 500",
    annualPrice: 219900,
    description: "Para negocios con catálogo amplio, más movimiento y hasta 500 productos.",
    limits: {
...
export const LANDING_PLAN_VALUES = ["basic", "commercial", "plus", "pro"];
...
export function resolvePrimaryModuleForBusinessCategory(category = "", businessType = "") {
  const profile = { businessCategory: category, businessType };
  if (isBookingEligibleBusiness(profile)) return "booking";
  if (isCommerceEligibleBusiness(profile)) return "commerce";
  return "";
}
...
export function resolveDefaultModuleAccess({ plan = "", businessCategory = "", businessType = "", selectedModule = "" } = {}) {
  const normalizedPlan = normalizeKlicorPlan(plan);
  const requestedModule = normalizeKlicorModule(selectedModule);
  const suggestedModule = requestedModule || resolvePrimaryModuleForBusinessCategory(businessCategory, businessType);
  const primaryModule = isBusinessModuleEligible({ businessCategory, businessType }, suggestedModule)
    ? suggestedModule
    : isCommerceEligibleBusiness({ businessCategory, businessType })
      ? "commerce"
      : "";
...
export function resolveUserModuleAccess(user = {}) {
  ...
  if (user.moduleAccess && typeof user.moduleAccess === "object") {
    ...
    return {
      commerce: stored.commerce && isBusinessModuleEligible(user, "commerce"),
      booking: stored.booking && isBusinessModuleEligible(user, "booking"),
    };
  }
...
export function canUseModule(user = {}, module = "") {
  ...
  if (!isBusinessModuleEligible(user, normalizedModule)) return false;
...
export function assertModuleAccess(user = {}, module = "") {
  ...
  if (normalizedModule && !isBusinessModuleEligible(user, normalizedModule)) {
...
export function getLandingPricingPlans() {
  return LANDING_PLAN_VALUES.map((planValue) => {
    ...
    const featuresByPlan = {
      ...
      pro: [
        "Todo lo del plan Comercial Plus",
        "Hasta 500 productos",
        "Mayor capacidad para catálogos amplios",
        "Pensado para negocios con más movimiento",
        "Uso normal incluido; alto flujo se cotiza aparte",
      ],
    };
