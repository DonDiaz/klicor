...
function mergeWorkingWindows(businessDay, staffDay) {
  if (!businessDay?.isOpen || !staffDay?.isWorking) return null;
  const startMinutes = Math.max(timeToMinutes(businessDay.startTime), timeToMinutes(staffDay.startTime));
  const endMinutes = Math.min(timeToMinutes(businessDay.endTime), timeToMinutes(staffDay.endTime));
...
function buildDaySlotOptions({
  ...
  const eligibleStaff = staffMembers.filter((staff) => {
    ...
    return Boolean(mergeWorkingWindows(businessDay, staffDay));
  });
  ...
  eligibleStaff.forEach((staff) => {
    const staffDay = getScheduleForDay(staff.schedule, dayOfWeek);
    const workingWindow = mergeWorkingWindows(businessDay, staffDay);
...
async function getAppointmentsByDate(uid, dateString, staffIds = []) {
  ...
  let query = appointmentsCollection(uid)
...
export async function createBookingAppointment(uid, rawInput, options = {}, user = null) {
  ...
  await getAdminDb().runTransaction(async (transaction) => {
    ...
    for (const candidateStaffId of candidateStaffIds) {
      const overlapQuery = appointmentsCollection(uid)
        .where("appointmentDate", "==", parsed.appointmentDate)
        .where("staffId", "==", candidateStaffId)
        .where("status", "in", BOOKING_ACTIVE_STATUS_VALUES)
        .orderBy("startMinutes");

      const [overlapSnap, staffSnap] = await Promise.all([
        transaction.get(overlapQuery),
        transaction.get(staffCollection(uid).doc(candidateStaffId)),
      ]);
      if (!staffSnap.exists) continue;

      const overlaps = overlapSnap.docs.map(normalizeAppointmentDoc);
