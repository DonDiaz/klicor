...
const BOOKING_STAFF_IMAGE_WIDTH = 720;
const BOOKING_STAFF_IMAGE_THUMB_SIZE = 192;
const BOOKING_IMAGE_CACHE_CONTROL = "public, max-age=31536000, immutable";
...
const BOOKING_CLOSE_REMINDER_LOOKBACK_DAYS = 30;
...
function invalidatePublicBookingCache({ currentUsername = "" } = {}) {
  ...
  revalidatePath(buildBookingPublicUrl(slug));
}
...
async function optimizeBookingStaffPhoto(uid, staffId, file) {
  ...
  const imageVersion = buildBookingId("img");
  const basePath = `booking-staff/${uid}/${staffId}/${imageVersion}`;
  ...
  await Promise.all([
    bucket.file(mainPath).save(mainBuffer, {
      metadata: {
        ...
        cacheControl: BOOKING_IMAGE_CACHE_CONTROL,
      },
      resumable: false,
      public: true,
    }),
...
async function optimizeBookingServicePhoto(uid, serviceId, file) {
  ...
  const bucket = getAdminStorage();
  const imageVersion = buildBookingId("img");
  const basePath = `booking-services/${uid}/${serviceId}/${imageVersion}`;
...
async function getAppointmentsToClose(uid, now = new Date()) {
  const today = getBogotaDateString(0, now);
  const startDate = getBogotaDateString(-BOOKING_CLOSE_REMINDER_LOOKBACK_DAYS, now);
  const { minutes: nowMinutes } = getBogotaNowParts(now);
  const appointments = await getAppointmentsByDateRange(uid, startDate, today);
...
export async function getBookingAdminState(uid, filters = {}, user = null) {
  ...
  const [appointments, appointmentsToClose] = await Promise.all([
    getAppointmentsByDate(uid, normalizedFilters.date, normalizedFilters.staffId ? [normalizedFilters.staffId] : []),
    getAppointmentsToClose(uid),
  ]);
  ...
  return {
    ...
    appointmentsToClose: appointmentsToClose.map((appointment) => buildAppointmentSummary(appointment, config)),
...
export async function createBookingAppointment(uid, rawInput, options = {}, user = null) {
  ...
  const legalConsent = options.legalConsent || null;
  ...
  await getAdminDb().runTransaction(async (transaction) => {
    ...
    transaction.set(ref, {
      ...
      customerLegalConsent: legalConsent ? {
        accepted: legalConsent.accepted === true,
        consentId: String(legalConsent.id || legalConsent.lastConsentId || "").trim(),
        termsVersion: String(legalConsent.termsVersion || "").trim(),
        privacyVersion: String(legalConsent.privacyVersion || "").trim(),
        source: String(legalConsent.source || "public_booking").trim(),
      } : null,
