...
export async function runBookingReminderSweep(now = new Date()) {
  ...
  for (const userDoc of usersSnap.docs) {
    ...
    if (!config.enabled || !config.reminderEnabled || !canUseModule(owner, "booking")) continue;
    ...
    for (const appointmentDoc of appointmentsSnap.docs) {
      ...
      if (data.reminderSentAt || data.reminderSkippedAt) continue;
      ...
      if (minutesUntil > reminderMinutes || minutesUntil < -BOOKING_REMINDER_SWEEP_WINDOW_MINUTES) continue;
...
async function safelySendBookingEmail(task) {
  try {
    await task();
  } catch (error) {
    console.error("[booking-email]", error?.message || error);
  }
}
...
async function notifyBusinessAboutNewBooking({ owner, config, appointment, summary, mode }) {
  if (config.notifyBusinessOnRequest === false || !owner?.email) return;

  await safelySendBookingEmail(() => sendBookingBusinessNotification({
    to: owner.email,
...
async function notifyCustomerConfirmed({ owner, config, appointment, summary }) {
  if (config.notifyCustomerOnConfirmation === false || !appointment?.customerEmail) return;

  await safelySendBookingEmail(() => sendBookingCustomerConfirmation({
...
async function notifyCustomerReminder({ owner, appointment, summary }) {
  ...
  await safelySendBookingEmail(() => sendBookingCustomerReminder({
    to: appointment.customerEmail,
    businessName: owner.businessName || "el negocio",
    appointment,
    summary,
...
async function notifyCustomerUpdated({ owner, config, appointment, summary, updateType }) {
  ...
  await safelySendBookingEmail(() => sendBookingCustomerUpdate({
    to: appointment.customerEmail,
    businessName: owner.businessName || "el negocio",
    appointment,
    summary,
...
export async function getBookingAvailability(uid, rawInput = {}, user = null) {
  ...
  const availableStaff = filterAvailableStaffForService(staff, service.id, selectedStaffId);
...
export async function createBookingAppointment(uid, rawInput, options = {}, user = null) {
  ...
  if (options.channel === "public") {
    await notifyBusinessAboutNewBooking({
      owner,
      config,
      appointment,
      summary,
    ...
    if (status === "confirmed") {
      await notifyCustomerConfirmed({ owner, config, appointment, summary });
    }
  }
...
export async function updateBookingAppointmentStatus(uid, rawInput, user = null) {
  ...
  if (savedSnap.exists) {
    ...
    if (parsed.status === "confirmed" && currentAppointment?.status !== "confirmed") {
      ...
      await notifyCustomerUpdated({ owner, config, appointment, summary, updateType: "cancelled" });
    }
  }
...
export async function updateBookingAppointmentSchedule(uid, rawInput, user = null) {
  ...
  await notifyCustomerUpdated({ owner, config, appointment, summary, updateType: "rescheduled" });
