...
export async function createCommerceSubcategory(uid, input, user = null) {
  ...
  const subcategoryIcon = resolveCommerceCategoryIcon(parsed.name);
  const iconKey = normalizeCommerceCategoryIconKey(parsed.iconKey || subcategoryIcon.iconKey);
  const color = normalizeCommerceCategoryColor(parsed.color || "");
  ...
  await ref.set({
    ...
    iconKey,
    color,
    iconSource: parsed.iconKey ? "manual" : subcategoryIcon.iconSource,
    iconMatchedAlias: subcategoryIcon.iconMatchedAlias,
    orderIndex,
    productCount: 0,
    visibleProductCount: 0,
