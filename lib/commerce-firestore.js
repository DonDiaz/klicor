...
import { getBusinessOpenStatus, normalizeBusinessHours } from "@/lib/business-hours";
import {
  buildCommerceCategoryNormalizedKey,
  normalizeCommerceCategoryColor,
  normalizeCommerceCategoryIconKey,
  resolveCommerceCategoryIcon,
} from "@/lib/commerce-category-icons";
...
const COMMERCE_IMAGE_WIDTH = 720;
const COMMERCE_IMAGE_CARD_SIZE = 480;
const COMMERCE_IMAGE_THUMB_SIZE = 192;
const COMMERCE_IMAGE_CACHE_CONTROL = "public, max-age=31536000, immutable";
const COMMERCE_PRODUCT_MAX_IMAGES = 4;
const PUBLIC_COMMERCE_INLINE_PRODUCT_LIMIT = 36;
...
function invalidatePublicCommerceCache({ currentUsername = "", mode = "" } = {}) {
  ...
  if (slug && normalizedMode) {
    revalidateTag(`public-commerce:${slug}:${normalizedMode}`);
    revalidatePath(buildCommercePublicUrl(slug, normalizedMode));
  }
}
...
function buildPublicStorageUrl(bucketName, path = "") {
  const cleanPath = String(path || "").trim();
  if (!bucketName || !cleanPath) return "";
  return `https://storage.googleapis.com/${bucketName}/${cleanPath.split("/").map(encodeURIComponent).join("/")}`;
}
...
function normalizeProductImages(value = [], fallback = {}) {
  const items = Array.isArray(value)
    ? value
      ...
      .map((item, index) => ({
        ...
        imageCardUrl: String(item.imageCardUrl || item.imageUrl || item.imageThumbUrl || "").trim(),
        imageThumbUrl: String(item.imageThumbUrl || item.imageUrl || "").trim(),
        imagePath: String(item.imagePath || "").trim(),
        imageCardPath: String(item.imageCardPath || item.imagePath || "").trim(),
      ...
      .filter((item) => item.imageUrl || item.imageCardUrl || item.imageThumbUrl)
    : [];
  ...
  return [{
    ...
    imageCardUrl: String(fallback.imageCardUrl || fallback.imageUrl || fallback.imageThumbUrl || "").trim(),
    imageThumbUrl: fallbackThumbUrl,
    imagePath: String(fallback.imagePath || "").trim(),
    imageCardPath: String(fallback.imageCardPath || fallback.imagePath || "").trim(),
    ...
    variants: fallback.variants && typeof fallback.variants === "object" ? fallback.variants : {},
  }];
}

function normalizeCategoryDoc(doc) {
  const data = doc.data();
  const icon = resolveCommerceCategoryIcon(data.name);
  const normalizedKey = buildCommerceCategoryNormalizedKey(data.normalizedKey || data.name);
  return {
    id: doc.id,
    name: String(data.name || "").trim(),
    slug: String(data.slug || "").trim(),
    normalizedKey,
    iconKey: String(data.iconKey || icon.iconKey).trim(),
    color: normalizeCommerceCategoryColor(data.color || ""),
    iconSource: String(data.iconSource || icon.iconSource).trim(),
    iconMatchedAlias: String(data.iconMatchedAlias || icon.iconMatchedAlias || "").trim(),
...
function hasVisiblePublicProducts(item = {}) {
  return Number(item.visibleProductCount || 0) > 0;
}
...
function normalizeProductDoc(doc) {
  ...
  return {
    ...
    featuredInDorika: Boolean(data.featuredInDorika),
    orderIndex: Number(data.orderIndex || 0) || 0,
    imageUrl: String(data.imageUrl || primaryImage?.imageUrl || "").trim(),
    imageCardUrl: String(data.imageCardUrl || primaryImage?.imageCardUrl || data.imageUrl || primaryImage?.imageUrl || "").trim(),
    ...
    imageCardPath: String(data.imageCardPath || primaryImage?.imageCardPath || data.imagePath || primaryImage?.imagePath || "").trim(),
...
async function resolveUniqueCommerceCategoryKey(uid, name, currentCategoryId = "") {
  const normalizedKey = buildCommerceCategoryNormalizedKey(name);
  if (!normalizedKey) {
    throw new Error("Ingresa un nombre de categoría válido.");
  }

  const snap = await categoriesCollection(uid).get();
  const duplicate = snap.docs
    .map(normalizeCategoryDoc)
    .find((category) => category.id !== currentCategoryId && category.normalizedKey === normalizedKey);

  if (duplicate) {
    throw new Error(`Ya existe la categoría "${duplicate.name}". Usa otro nombre para evitar duplicados.`);
  }

  return normalizedKey;
}

async function resolveUniqueCommerceSubcategoryKey(uid, categoryId, name, currentSubcategoryId = "") {
  const normalizedKey = buildCommerceCategoryNormalizedKey(name);
  if (!normalizedKey) {
    throw new Error("Ingresa un nombre de subcategoría válido.");
  }

  const snap = await subcategoriesCollection(uid).where("categoryId", "==", categoryId).get();
  const duplicate = snap.docs
    .map(normalizeSubcategoryDoc)
    .find((subcategory) => subcategory.id !== currentSubcategoryId && subcategory.normalizedKey === normalizedKey);

  if (duplicate) {
    throw new Error(`Ya existe la subcategoría "${duplicate.name}" en esta categoría. Usa otro nombre para evitar duplicados.`);
  }
  ...
  return {
    ...
    subcategories: Array.isArray(data.subcategories)
      ? data.subcategories.map((item) => {
        const icon = resolveCommerceCategoryIcon(item.name);
        return {
          id: String(item.id || "").trim(),
          categoryId: String(item.categoryId || "").trim(),
        name: String(item.name || "").trim(),
        slug: String(item.slug || "").trim(),
        normalizedKey: buildCommerceCategoryNormalizedKey(item.normalizedKey || item.name),
        iconKey: String(item.iconKey || icon.iconKey).trim(),
          color: normalizeCommerceCategoryColor(item.color || ""),
        iconSource: String(item.iconSource || icon.iconSource).trim(),
        iconMatchedAlias: String(item.iconMatchedAlias || icon.iconMatchedAlias || "").trim(),
      ...
      }).filter((item) => item.id && isPublicVisible(item) && hasVisiblePublicProducts(item))
      : [],
    products: pagination.items,
    previewImage: data.previewImage && typeof data.previewImage === "object"
      ? {
        imageUrl: String(data.previewImage.imageUrl || "").trim(),
        imageThumbUrl: String(data.previewImage.imageThumbUrl || data.previewImage.imageUrl || "").trim(),
      }
      : null,
...
function shouldPreloadPublicSections(config = {}) {
  const productsCount = Number(config.visibleProductsCount || config.productsCount || 0) || 0;
...
function toPublicProductSummary(product) {
  return {
    ...
    images: normalizeProductImages(product.images, product).map((image) => ({
      ...
      imageCardUrl: image.imageCardUrl,
...
function resolveFirstPublicProductImage(products = []) {
  for (const product of products) {
    const image = normalizeProductImages(product?.images, product)[0];
    const imageUrl = String(image?.imageUrl || product?.imageUrl || "").trim();
  const imageCardUrl = String(image?.imageCardUrl || product?.imageCardUrl || imageUrl).trim();
  const imageThumbUrl = String(image?.imageThumbUrl || product?.imageThumbUrl || imageCardUrl || imageUrl).trim();
  if (imageUrl || imageCardUrl || imageThumbUrl) {
    return {
        imageUrl: imageUrl || imageCardUrl || imageThumbUrl,
        imageCardUrl: imageCardUrl || imageUrl || imageThumbUrl,
...
async function syncPublicCommerceSections(uid) {
  ...
  const categories = categorySnap.docs.map(normalizeCategoryDoc).filter(isPublicVisible);
  const subcategories = subcategorySnap.docs.map(normalizeSubcategoryDoc).filter(isPublicVisible);
  ...
  categories.forEach((category) => {
    const sectionSubcategories = (subcategoriesByCategory.get(category.id) || [])
      .filter((subcategory) => isPublicVisible(subcategory) && (hasVisiblePublicProducts(subcategory) || (productsBySubcategory.get(subcategory.id) || []).length > 0))
      .sort((left, right) => left.orderIndex - right.orderIndex);
    const categoryPreviewImage = resolveFirstPublicProductImage((productsByCategory.get(category.id) || []).sort((left, right) => left.orderIndex - right.orderIndex));
    if (sectionSubcategories.length) {
      ...
      queueSection(buildPublicCategorySectionId(category.id), {
        ...
        previewImage: categoryPreviewImage,
...
function getProductStoragePaths(product = {}) {
  const galleryPaths = normalizeProductImages(product.images, product).flatMap((image) => [image.imagePath, image.imageCardPath, image.imageThumbPath]);
  return [...new Set([
    product.imagePath,
...
function getImageStoragePaths(images = []) {
  return [...new Set(
    normalizeProductImages(images)
      .flatMap((image) => [image.imagePath, image.imageCardPath, image.imageThumbPath])
      .filter(Boolean),
  )];
}
...
async function optimizeCommerceProductImage(uid, productId, file, imageId = buildCommerceId("img")) {
  ...
  const cardBuffer = await image
    .clone()
    .resize(COMMERCE_IMAGE_CARD_SIZE, COMMERCE_IMAGE_CARD_SIZE, {
      fit: "cover",
      position: "centre",
    })
    .webp({ quality: 80 })
  ...
  await Promise.all([
    bucket.file(mainPath).save(mainBuffer, {
      metadata: {
        ...
        cacheControl: COMMERCE_IMAGE_CACHE_CONTROL,
      },
      resumable: false,
      public: true,
    }),
    bucket.file(cardPath).save(cardBuffer, {
      metadata: {
        contentType: "image/webp",
        cacheControl: COMMERCE_IMAGE_CACHE_CONTROL,
      },
  ...
  return {
    id: imageId,
    imageUrl: buildPublicStorageUrl(bucket.name, mainPath),
    imageCardUrl: buildPublicStorageUrl(bucket.name, cardPath),
    imageThumbUrl: buildPublicStorageUrl(bucket.name, thumbPath),
    imagePath: mainPath,
    imageCardPath: cardPath,
    imageThumbPath: thumbPath,
    imageWidth: Number(metadata.width || 0) || 0,
    imageHeight: Number(metadata.height || 0) || 0,
    variants: {
      thumb: {
        width: COMMERCE_IMAGE_THUMB_SIZE,
        height: COMMERCE_IMAGE_THUMB_SIZE,
        bytes: thumbBuffer.length,
        contentType: "image/webp",
        path: thumbPath,
        url: buildPublicStorageUrl(bucket.name, thumbPath),
      },
      card: {
        width: COMMERCE_IMAGE_CARD_SIZE,
        height: COMMERCE_IMAGE_CARD_SIZE,
        bytes: cardBuffer.length,
        contentType: "image/webp",
        path: cardPath,
        url: buildPublicStorageUrl(bucket.name, cardPath),
      },
      large: {
        width: Number(metadata.width || 0) > COMMERCE_IMAGE_WIDTH ? COMMERCE_IMAGE_WIDTH : Number(metadata.width || COMMERCE_IMAGE_WIDTH),
        height: Number(metadata.height || 0) || 0,
        bytes: mainBuffer.length,
        contentType: "image/webp",
        path: mainPath,
        url: buildPublicStorageUrl(bucket.name, mainPath),
      },
    },
  };
}
...
function buildPublicCommerceViewUser(user = {}) {
  return {
    ...
    commerceMode: user.commerceMode || user.commerce?.activeMode || "",
    photo: user.photo || "",
...
export async function getCommerceAdminState(uid, user = null) {
  ...
  const products = productSnap.docs.map(normalizeProductDoc).sort((left, right) => left.orderIndex - right.orderIndex);
  const categoriesWithImages = applyCategoryImagesFromProducts(categories, products);
  ...
  return {
    ...
    categories: categoriesWithImages.map((category) => ({
      ...category,
...
export async function getCommerceAdminStructure(uid, user = null) {
  ...
  const categories = categorySnap.docs.map(normalizeCategoryDoc);
  const subcategories = subcategorySnap.docs.map(normalizeSubcategoryDoc);
  const categoryPreviewSections = await readPublicCategoryPreviewSections(uid, categories);
  const categoriesWithImages = applyPublicCategoryImages(categories, categoryPreviewSections);
  const groupedSubcategories = new Map();
...
export async function createCommerceCategory(uid, input, user = null) {
  ...
  const normalizedKey = await resolveUniqueCommerceCategoryKey(uid, parsed.name);
  const categoryIcon = resolveCommerceCategoryIcon(parsed.name);
  const iconKey = normalizeCommerceCategoryIconKey(parsed.iconKey || categoryIcon.iconKey);
  const color = normalizeCommerceCategoryColor(parsed.color || "");
  ...
  await ref.set({
    ...
    iconKey,
    color,
    iconSource: parsed.iconKey ? "manual" : categoryIcon.iconSource,
    iconMatchedAlias: categoryIcon.iconMatchedAlias,
    orderIndex,
...
export async function updateCommerceCategory(uid, input, user = null) {
  ...
  const categoryRef = categoriesCollection(uid).doc(parsed.id);
  const currentSnap = await categoryRef.get();
  if (!currentSnap.exists) throw new Error("Categoría no encontrada.");
  const normalizedKey = await resolveUniqueCommerceCategoryKey(uid, parsed.name, parsed.id);
  const categoryIcon = resolveCommerceCategoryIcon(parsed.name);
  const iconKey = normalizeCommerceCategoryIconKey(parsed.iconKey || categoryIcon.iconKey);
  const color = normalizeCommerceCategoryColor(parsed.color || "");
  await categoryRef.set({
...
export async function createCommerceSubcategory(uid, input, user = null) {
  ...
  const normalizedKey = await resolveUniqueCommerceSubcategoryKey(uid, parsed.categoryId, parsed.name);
...
export async function updateCommerceSubcategory(uid, input, user = null) {
  ...
  const normalizedKey = await resolveUniqueCommerceSubcategoryKey(uid, parsed.categoryId, parsed.name, parsed.id);

  const movedCategory = current.categoryId !== parsed.categoryId;
  ...
  if (movedCategory) {
    const productSnap = await productsCollection(uid).where("subcategoryId", "==", parsed.id).get();
    await Promise.all(productSnap.docs.map((doc) => (
      doc.ref.set({
        categoryId: parsed.categoryId,
        updatedAt: FieldValue.serverTimestamp(),
      }, { merge: true })
...
export async function saveCommerceProduct(uid, rawInput, assets = {}, user = null) {
  ...
  const parsed = commerceProductSchema.parse({
    ...
    available: rawInput.available !== false,
  });
  ...
  const removedImageIds = new Set(Array.isArray(parsed.removedImageIds) ? parsed.removedImageIds : []);
  const removedImages = productImages.filter((image) => removedImageIds.has(image.id));
  if (removedImageIds.size) {
    productImages = productImages.filter((image) => !removedImageIds.has(image.id));
  }

  const incomingImages = Array.isArray(assets.images) ? assets.images.filter(Boolean) : [];
  if (productImages.length + incomingImages.length > COMMERCE_PRODUCT_MAX_IMAGES) {
    throw new Error(`Puedes guardar máximo ${COMMERCE_PRODUCT_MAX_IMAGES} fotos por producto.`);
  }

  if (incomingImages.length) {
    const uploadedImages = await Promise.all(
      incomingImages.map((file, index) => optimizeCommerceProductImage(uid, ref.id, file, `${String(index + 1).padStart(2, "0")}_${buildCommerceId("img")}`)),
    );
    productImages = [...productImages, ...uploadedImages.filter(Boolean)];
  ...
  await ref.set({
    ...
    featuredInDorika: Boolean(parsed.featuredInDorika),
    orderIndex,
    imageUrl: primaryImage?.imageUrl || "",
    imageCardUrl: primaryImage?.imageCardUrl || primaryImage?.imageUrl || "",
    imageThumbUrl: primaryImage?.imageThumbUrl || primaryImage?.imageUrl || "",
    imagePath: primaryImage?.imagePath || "",
    imageCardPath: primaryImage?.imageCardPath || primaryImage?.imagePath || "",
  ...
  if (removedImages.length) {
    await removeStoragePaths(getImageStoragePaths(removedImages));
  }
...
async function readPublicCategories(uid) {
  ...
  return snap.docs.map(normalizeCategoryDoc).filter((category) => isPublicVisible(category) && hasVisiblePublicProducts(category));
}

async function readPublicCategoryPreviewSections(uid, categories = []) {
  if (!categories.length) return [];
  const snaps = await Promise.all(
    categories.map((category) => publicSectionsCollection(uid).doc(buildPublicCategorySectionId(category.id)).get()),
  );
  return snaps.filter((snap) => snap.exists).map(normalizePublicSectionDoc);
}

function applyPublicCategoryImages(categories = [], sections = []) {
  const sectionsByCategory = new Map();
  sections.forEach((section) => {
    if (section.type === "category" && section.categoryId) {
      sectionsByCategory.set(section.categoryId, section);
    }
  });

  return categories.map((category) => {
    if (category.imageUrl || category.imageThumbUrl) return category;
    const section = sectionsByCategory.get(category.id);
    const sectionPreview = section?.previewImage?.imageUrl || section?.previewImage?.imageThumbUrl
      ? section.previewImage
      : resolveFirstPublicProductImage(section?.products || []);
    if (!sectionPreview) return category;
    return {
      ...category,
      imageUrl: sectionPreview.imageUrl || sectionPreview.imageThumbUrl || "",
      imageThumbUrl: sectionPreview.imageThumbUrl || sectionPreview.imageUrl || "",
    };
  });
}

function applyCategoryImagesFromProducts(categories = [], products = []) {
  const productsByCategory = new Map();
  products.forEach((product) => {
  ...
  return categories.map((category) => {
    if (category.imageUrl || category.imageThumbUrl) return category;
    const preview = resolveFirstPublicProductImage((productsByCategory.get(category.id) || []).sort((left, right) => left.orderIndex - right.orderIndex));
    if (!preview) return category;
    return {
      ...category,
      imageUrl: preview.imageUrl || preview.imageThumbUrl || "",
      imageThumbUrl: preview.imageThumbUrl || preview.imageUrl || "",
    };
  });
}
...
async function readPublicSubcategories(uid, categoryId) {
  const snap = await subcategoriesCollection(uid)
    ...
    .orderBy("orderIndex")
    .get();
  return snap.docs.map(normalizeSubcategoryDoc).filter((subcategory) => isPublicVisible(subcategory) && hasVisiblePublicProducts(subcategory));
}

async function readPublicProducts(uid, { categoryId = "", subcategoryId = "", limit = COMMERCE_PRODUCT_PAGE_SIZE, after = null } = {}) {
  if (categoryId) {
    const categorySnap = await categoriesCollection(uid).doc(categoryId).get();
    if (!categorySnap.exists || !isPublicVisible(normalizeCategoryDoc(categorySnap))) {
      return { items: [], hasMore: false, nextCursor: null };
    }
  }
  if (subcategoryId) {
    const subcategorySnap = await subcategoriesCollection(uid).doc(subcategoryId).get();
    if (!subcategorySnap.exists || !isPublicVisible(normalizeSubcategoryDoc(subcategorySnap))) {
      return { items: [], hasMore: false, nextCursor: null };
    }
  }
...
async function readPublicProductById(uid, productId = "", mode = "") {
  const cleanProductId = String(productId || "").trim();
  if (!cleanProductId) return null;

  const snap = await productsCollection(uid).doc(cleanProductId).get();
  if (!snap.exists) return null;

  const product = normalizeProductDoc(snap);
  if (!product.visible) return null;
  if (mode && product.mode !== mode) return null;
  const [categorySnap, subcategorySnap] = await Promise.all([
    product.categoryId ? categoriesCollection(uid).doc(product.categoryId).get() : null,
    product.subcategoryId ? subcategoriesCollection(uid).doc(product.subcategoryId).get() : null,
  ]);
  if (!categorySnap?.exists || !isPublicVisible(normalizeCategoryDoc(categorySnap))) return null;
  if (subcategorySnap && (!subcategorySnap.exists || !isPublicVisible(normalizeSubcategoryDoc(subcategorySnap)))) return null;
...
export async function getPublicCommerceBootstrap(owner) {
  ...
  const experience = resolveCommerceExperience({ activeMode }, owner);
  if (!activeMode || !canUseModule(owner, "commerce")) return null;
  const businessHoursStatus = getBusinessOpenStatus(owner.businessHours);

  const categories = await readPublicCategories(owner.uid);
  const shouldPreloadSections = shouldPreloadPublicSections(config);
  const allSections = shouldPreloadSections ? await readAllPublicSectionSnapshots(owner.uid) : [];
  const categoryPreviewSections = shouldPreloadSections ? allSections : await readPublicCategoryPreviewSections(owner.uid, categories);
  const publicCategories = applyPublicCategoryImages(categories, categoryPreviewSections);
  const prefetchedSections = shouldPreloadSections ? buildPrefetchedPublicSections(allSections) : [];
  const firstCategory = publicCategories[0] || null;
...
export async function getPublicCommerceProductDeepLink(owner, { productId = "", mode = "" } = {}) {
  const activeMode = normalizeCommerceMode(mode || owner?.commerceMode || owner?.commerce?.activeMode);
  if (!owner?.uid || !activeMode) return null;

  const product = await readPublicProductById(owner.uid, productId, activeMode);
  if (!product) return null;

  const productSummary = toPublicProductSummary(product);
  const chunk = await getPublicCommerceChunk(owner.uid, {
    categoryId: product.categoryId,
    subcategoryId: product.subcategoryId,
    includeSubcategories: true,
  });
  const chunkProducts = Array.isArray(chunk?.products) ? chunk.products : [];
  const hasProductInChunk = chunkProducts.some((item) => String(item.id) === String(productSummary.id));
  ...
  return {
    ...
    products: hasProductInChunk ? chunkProducts : [productSummary, ...chunkProducts],
  };
}
