...
function resolveFirstPublicProductImage(products = []) {
  for (const product of products) {
    ...
    const imageThumbUrl = String(image?.imageThumbUrl || product?.imageThumbUrl || imageUrl).trim();
    if (imageUrl || imageThumbUrl) {
      return {
        imageUrl: imageUrl || imageThumbUrl,
...
function getImageStoragePaths(images = []) {
  return [...new Set(
    normalizeProductImages(images)
      .flatMap((image) => [image.imagePath, image.imageThumbPath])
      .filter(Boolean),
  )];
}
