...
function normalizePublicSectionDoc(doc) {
  ...
  return {
    ...
    subcategories: Array.isArray(data.subcategories)
      ...
      }).filter((item) => item.id && hasVisiblePublicProducts(item))
      : [],
...
async function syncPublicCommerceSections(uid) {
  ...
  categories.forEach((category) => {
    const sectionSubcategories = (subcategoriesByCategory.get(category.id) || [])
      .filter((subcategory) => hasVisiblePublicProducts(subcategory) || (productsBySubcategory.get(subcategory.id) || []).length > 0)
...
async function readPublicCategories(uid) {
  ...
  return snap.docs.map(normalizeCategoryDoc).filter(hasVisiblePublicProducts);
}
...
async function readPublicSubcategories(uid, categoryId) {
  const snap = await subcategoriesCollection(uid)
    ...
    .orderBy("orderIndex")
    .get();
  return snap.docs.map(normalizeSubcategoryDoc).filter(hasVisiblePublicProducts);
}
