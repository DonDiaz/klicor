...
const PUBLIC_COMMERCE_CACHE_VERSION = "module-access-v2";
...
export async function getPublicCommerceBootstrapByUsername(username, mode) {
  ...
  return unstable_cache(
    ...
    ["public-commerce-bootstrap", PUBLIC_COMMERCE_CACHE_VERSION, slug, normalizedMode],
    {
...
export async function getPublicCommerceChunkByUsername(username, mode, selection = {}) {
  ...
  return unstable_cache(
    async () => {
      const owner = await getUserByUsername(slug);
      if (!owner) return null;
      if (!canUseModule(owner, "commerce")) return null;
    ...
    ["public-commerce-chunk", PUBLIC_COMMERCE_CACHE_VERSION, slug, normalizedMode, categoryId, subcategoryId, after, includeSubcategories ? "with-subcategories" : "products-only"],
    {
      tags: [`public-commerce:${slug}`, `public-commerce:${slug}:${normalizedMode}`],
      revalidate: 300,
    },
...
export async function getPublicCommerceProductDeepLinkByUsername(username, mode, productId = "") {
  ...
  return unstable_cache(
    ...
    ["public-commerce-product-deeplink", PUBLIC_COMMERCE_CACHE_VERSION, slug, normalizedMode, cleanProductId],
    {
      tags: [`public-commerce:${slug}`, `public-commerce:${slug}:${normalizedMode}`],
      revalidate: 300,
    },
