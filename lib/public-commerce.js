...
import { getPublicCommerceBootstrap, getPublicCommerceChunk } from "@/lib/commerce-firestore";
...
export async function getPublicCommerceBootstrapByUsername(username, mode) {
  ...
  if (!slug || !normalizedMode) return null;
...
export async function getPublicCommerceChunkByUsername(username, mode, selection = {}) {
  ...
  return unstable_cache(
    async () => {
      ...
      return getPublicCommerceChunk(owner.uid, { categoryId, subcategoryId, after });
    },
    ["public-commerce-chunk", slug, normalizedMode, categoryId, subcategoryId, after],
    {
      tags: [`public-commerce:${slug}`, `public-commerce:${slug}:${normalizedMode}`],
      revalidate: 300,
    },
