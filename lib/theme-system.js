...
export const APPEARANCE_SWATCHES = {
  ...
  buttonPrimaryTextColor: ["#FFFFFF", "#0B1020", "#F8FAFC", "#111827"],
  buttonSecondaryTextColor: ["#FFFFFF", "#0B1020", "#F8FAFC", "#111827"],
  buttonTertiaryTextColor: ["#FFFFFF", "#0B1020", "#F8FAFC", "#111827"],
};
...
export function pickReadableTextColor(background, preferred = "#FFFFFF", fallback = "#0B1020") {
  const candidates = unique([preferred, fallback, "#FFFFFF", "#0B1020"])
    .map((color) => clampHex(color, ""))
    .filter(Boolean);
  let bestColor = candidates[0] || "#FFFFFF";
  let bestRatio = 0;
...
export function normalizeAppearance(input = {}) {
  ...
  const buttonPrimaryTextColor = pickReadableTextColor(
    primaryColor,
    input?.buttonPrimaryTextColor || input?.primaryButtonTextColor || legacyButtonText || preset.buttonPrimaryTextColor || preset.buttonTextColor,
    "#FFFFFF",
  );
  const buttonSecondaryTextColor = pickReadableTextColor(
    secondaryColor,
    input?.buttonSecondaryTextColor || input?.secondaryButtonTextColor || preset.buttonSecondaryTextColor || legacyButtonText || buttonPrimaryTextColor,
    buttonPrimaryTextColor,
  );
  const buttonTertiaryTextColor = pickReadableTextColor(
    tertiaryColor,
    input?.buttonTertiaryTextColor || input?.tertiaryButtonTextColor || preset.buttonTertiaryTextColor || legacyButtonText || "#0B1020",
    "#0B1020",
  );
  ...
  return {
    ...
    buttonTextColor: buttonPrimaryTextColor,
    buttonPrimaryTextColor,
    ...
    socialStyle: clampEnum(input?.socialStyle === "brand-circles" ? "icons" : input?.socialStyle, ["cards", "icons"], preset.socialStyle || APPEARANCE_DEFAULTS.socialStyle),
...
export function getAppearanceSuggestions(appearance) {
  ...
  const checks = [
    {
      key: "button-primary",
      ratio: getContrastRatio(next.primaryColor, next.buttonPrimaryTextColor),
      min: 4.5,
      message: "Sugerencia: el contraste del botón principal podría mejorarse.",
    },
    {
      key: "button-secondary",
      ratio: getContrastRatio(next.secondaryColor, next.buttonSecondaryTextColor),
      min: 4.5,
      message: "Sugerencia: el contraste del botón de prioridad 2 podría mejorarse.",
    },
    {
      key: "button-tertiary",
      ratio: getContrastRatio(next.tertiaryColor, next.buttonTertiaryTextColor),
      min: 4.5,
      message: "Sugerencia: el contraste del botón de prioridad 3 podría mejorarse.",
    },
  ];
