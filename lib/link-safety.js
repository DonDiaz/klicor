...
function buildBlockedError(link, reason) {
  const error = new Error(`Bloqueamos el enlace "${getLinkName(link)}" porque ${reason}.`);
  error.code = "PROFILE_LINK_BLOCKED";
  error.linkSafety = {
    reason,
    linkId: String(link.id || "").slice(0, 120),
    linkType: String(link.type || "").slice(0, 60),
    linkLabel: getLinkName(link).slice(0, 80),
    url: String(link.url || link.value || "").slice(0, 300),
  };
  return error;
}
...
function runLocalUrlChecks(link) {
  ...
  if (!["http:", "https:"].includes(parsed.protocol)) {
    throw buildBlockedError(link, "solo admite direcciones web públicas");
  }

  if (parsed.username || parsed.password) {
    throw buildBlockedError(link, "incluye credenciales dentro de la dirección");
  }

  if (BLOCKED_HOSTS.has(hostname) || BLOCKED_HOST_SUFFIXES.some((suffix) => hostname.endsWith(suffix))) {
    throw buildBlockedError(link, "apunta a una dirección local o privada");
  }
