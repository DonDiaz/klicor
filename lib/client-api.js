...
async function readAppCheckToken() {
  if (typeof window === "undefined") return "";
  try {
    const { getClientAppCheckToken } = await import("@/lib/firebase-client");
    return await getClientAppCheckToken();
  } catch {
    return "";
  }
}
...
export async function apiFetch(path, { token, body, method = "GET", isFormData = false, cache } = {}) {
