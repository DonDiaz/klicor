...
import { sendAgencyAccessRequestEmail, sendAgencyAccessResponseEmail } from "@/lib/mailer";
...
export async function recordAgencyEdit(access = {}, action = "") {
  const businessUid = String(access?.business?.uid || "").trim();
  if (!businessUid || !access?.agency?.id) return;

  await usersCollection().doc(businessUid).set({
    updatedBy: access.agency.id,
    updatedByRole: "agency",
    updatedByEmail: access.agency.email || "",
    updatedByName: access.agency.agencyName || access.agency.email || "",
    updatedForUid: businessUid,
    agencyLastEdit: {
      action: String(action || "agency_edit").trim(),
      agencyId: access.agency.id,
      agencyEmail: access.agency.email || "",
      agencyName: access.agency.agencyName || access.agency.email || "",
...
export async function respondAgencyAccessRequest({ businessUser, requestId, action } = {}) {
  ...
  if (finalStatus === "accepted" || finalStatus === "rejected") {
    ...
    sendAgencyAccessResponseEmail({
      to: request.agencyEmail,
      businessName: request.businessName,
      businessEmail: request.businessEmail,
      agencyName: request.agencyName,
      accepted: finalStatus === "accepted",
    }).catch((error) => {
      console.error("[agency-response-email]", error?.message || error);
    });
  }
