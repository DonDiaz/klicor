...
function estimateAnnualValue(user = {}, settings = ADMIN_SETTINGS_DEFAULTS) {
  ...
  if (plan === "agency") {
    return settings.agencyAnnualPrice || settings.annualPrice;
  }

  if (plan === "institutional") {
    return settings.partnerDefaultPrice || settings.annualPrice;
  }

  return settings.annualPrice;
}
...
function buildAdminUserSummary(user = {}, settings = ADMIN_SETTINGS_DEFAULTS) {
  ...
  return {
    ...
    businessTypeLabel: resolveBusinessCategoryLabel(account.businessCategory),
    origin,
    ...
    publicUrl: account.usernameLower ? buildVanityProfileUrl(account.usernameLower) : buildStableProfileUrl(account.publicLinkId),
...
export async function getAdminPanelSnapshot() {
  const [settings, usersSnap, partnersSnap] = await Promise.all([
    getAdminSettings(),
    usersCollection().get(),
  ...
  const users = usersSnap.docs
    .map((doc) => buildAdminUserSummary({ uid: doc.id, ...doc.data() }, settings))
...
export async function getAdminUserDetail(uid) {
  ...
  const summary = buildAdminUserSummary(rawUser, settings);
...
export async function updateAdminUserDetails(uid, input, actor) {
...
export async function updateAdminUserAccess(uid, input, actor) {
...
export async function extendAdminUserAccess(uid, days, actor) {
  const ref = usersCollection().doc(uid);
  const snap = await ref.get();
  if (!snap.exists) {
    throw new Error("Usuario no encontrado");
...
export async function registerAdminManualPayment(uid, input, actor) {
  const settings = await getAdminSettings();
  ...
  await getAdminDb().runTransaction(async (transaction) => {
    ...
    const plan = normalizePlan(input.plan || current.plan || "annual");
  ...
  await logAdminAction({
    ...
    metadata: {
      ...
      plan: normalizePlan(input.plan || "annual"),
    },
  });
