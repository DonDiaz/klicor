...
export async function listAdminUsersPage({
  ...
  while (items.length < resolvedPageSize && batches < 5) {
    ...
    snap.docs.forEach((doc) => {
      ...
      const summary = buildAdminUserSummary({ uid: doc.id, ...doc.data() }, settings, baseUrl);
...
export async function getAdminPanelSnapshot({ baseUrl = "", usersQuery = {} } = {}) {
  ...
  const [metrics, usersPage, dueTodayPage, dueSoonPage, overduePage] = await Promise.all([
    ...
    listAdminUsersPage({ baseUrl, ...usersQuery }),
    listAdminUsersPage({ baseUrl, pageSize: 10, renewal: "today", sort: "expires_asc" }),
    listAdminUsersPage({ baseUrl, pageSize: 10, renewal: "soon", sort: "expires_asc" }),
    listAdminUsersPage({ baseUrl, pageSize: 10, renewal: "overdue", sort: "expires_asc" }),
  ]);
