...
function commercialUsageCollection() {
  return getAdminDb().collection("commercialUsage");
}
...
function getCommercialUsageMonthKey(date = new Date()) {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
}

function resolveHighFlowThreshold(plan = "") {
  const normalizedPlan = normalizePlan(plan);
  if (normalizedPlan === "commercial") return 300;
  if (normalizedPlan === "plus") return 800;
  if (normalizedPlan === "pro") return 1500;
  return 120;
}

function buildAdminUserSummary(user = {}, settings = ADMIN_SETTINGS_DEFAULTS, baseUrl = "", usageByUid = new Map()) {
  ...
  const daysToExpiry = expiresAt
    ...
    : null;
  const usage = usageByUid.get(account.uid) || null;
  ...
  return {
    ...
    highFlow: Boolean(usage?.highFlow),
  };
}

function clampPageSize(value = 25) {
  const next = Number(value || 25);
  if (!Number.isFinite(next)) return 25;
  return Math.min(Math.max(Math.round(next), 10), 100);
}

function normalizeAdminSort(value = "") {
  return ["created_asc", "created_desc", "expires_asc", "expires_desc", "name"].includes(value)
    ? value
    : "created_desc";
}

function buildAdminUsersBaseQuery({ sort = "created_desc" } = {}) {
  let query = usersCollection();

  switch (normalizeAdminSort(sort)) {
    case "created_asc":
      return query.orderBy("createdAt", "asc");
    case "expires_asc":
      return query.orderBy("expiresAt", "asc");
    case "expires_desc":
      return query.orderBy("expiresAt", "desc");
    case "name":
      return query.orderBy("businessName", "asc");
    case "created_desc":
    default:
...
function matchesAdminExactFilters(user = {}, { origin = "all", accountStatus = "all", plan = "all" } = {}) {
  if (origin && origin !== "all" && user.origin !== normalizeOrigin(origin)) return false;
  if (accountStatus && accountStatus !== "all" && user.accountStatus !== normalizeAccountStatus(accountStatus)) return false;
  if (plan && plan !== "all" && user.plan !== normalizePlan(plan)) return false;
  return true;
}

function matchesAdminSearch(user = {}, search = "") {
...
function matchesAdminRenewalFilter(user = {}, filter = "all") {
...
async function buildAdminMetricsSnapshot(settings = ADMIN_SETTINGS_DEFAULTS) {
  const now = new Date();
  const renewalLimit = addDaysFrom(now, Number(settings.renewalAlertDays || 7));
  const monthRanges = [];
  ...
  const [
    ...
    countQuery(usersCollection()),
    countQuery(usersCollection().where("accountStatus", "==", "trial")),
    countQuery(usersCollection().where("accountStatus", "==", "active")),
    countQuery(usersCollection().where("accountStatus", "==", "expired")),
    countQuery(usersCollection().where("accountStatus", "==", "suspended")),
    countQuery(usersCollection().where("accountStatus", "==", "pending_payment")),
    countQuery(usersCollection().where("expiresAt", ">=", Timestamp.fromDate(now)).where("expiresAt", "<=", Timestamp.fromDate(renewalLimit))),
    Promise.all(ADMIN_ORIGIN_OPTIONS.map(async (option) => ({
      key: option.value,
      label: option.label,
      total: await countQuery(usersCollection().where("origin", "==", option.value)),
    }))),
    Promise.all(ADMIN_ACCOUNT_STATUS_OPTIONS.map(async (option) => ({
      key: option.value,
      label: option.label,
      total: await countQuery(usersCollection().where("accountStatus", "==", option.value)),
    }))),
    Promise.all(monthRanges.map(async (item) => ({
      key: item.key,
      label: item.label,
      total: await countQuery(usersCollection().where("createdAt", ">=", Timestamp.fromDate(item.start)).where("createdAt", "<", Timestamp.fromDate(item.end))),
    }))),
  ]);
  ...
  return {
    ...
    estimatedAnnualRevenueLabel: "Ver cobros",
    byOrigin,
    byStatus,
    monthlyGrowth,
  };
...
async function buildHighFlowSummary({ monthKey = getCommercialUsageMonthKey(), limit = 200 } = {}) {
  const snapshot = await commercialUsageCollection()
  ...
  const items = snapshot.docs
    .map((doc) => {
      ...
      const threshold = resolveHighFlowThreshold(plan);
      const total = Number(data.total || 0);
      return {
        id: doc.id,
        uid: String(data.uid || ""),
        username: String(data.username || ""),
        businessName: String(data.businessName || "Sin nombre"),
        plan,
        planLabel: resolvePlanLabel(plan),
        monthKey,
        total,
        threshold,
        highFlow: total >= threshold,
        status: total >= threshold ? "review" : "normal",
  ...
  items.forEach((item) => {
    if (item.uid) usageByUid.set(item.uid, item);
  });

  const toReview = items.filter((item) => item.highFlow);

  return {
    monthKey,
    totalTracked: items.length,
    totalToReview: toReview.length,
    items: toReview.slice(0, 8),
    usageByUid,
  };
}
...
export async function listAdminUsersPage({
  ...
  const scanLimit = Math.min(Math.max(resolvedPageSize * 3, 30), 150);
  const items = [];
  let query = buildAdminUsersBaseQuery({ sort });
  let lastScannedDoc = null;
  let batches = 0;

  if (cursor) {
    const cursorSnap = await usersCollection().doc(String(cursor)).get().catch(() => null);
    if (cursorSnap?.exists) {
      query = query.startAfter(cursorSnap);
    }
  }

  while (items.length < resolvedPageSize && batches < 5) {
    ...
    snap.docs.forEach((doc) => {
      if (items.length >= resolvedPageSize) return;
      const summary = buildAdminUserSummary({ uid: doc.id, ...doc.data() }, settings, baseUrl, usageByUid);
      if (!matchesAdminExactFilters(summary, { origin, accountStatus, plan })) return;
      if (!matchesAdminSearch(summary, search)) return;
      if (!matchesAdminRenewalFilter(summary, renewal)) return;
      items.push(summary);
    });

    if (snap.docs.length < scanLimit || items.length >= resolvedPageSize) break;
    query = buildAdminUsersBaseQuery({ sort }).startAfter(lastScannedDoc);
  }

  return {
    items,
    pageSize: resolvedPageSize,
    nextCursor: lastScannedDoc && items.length >= resolvedPageSize ? lastScannedDoc.id : "",
    hasNextPage: Boolean(lastScannedDoc && items.length >= resolvedPageSize),
    filters: {
...
export async function getAdminPanelSnapshot({ baseUrl = "", usersQuery = {} } = {}) {
  const [settings, partnersSnap, agencies] = await Promise.all([
    getAdminSettings(),
    partnersCollection().limit(50).get().catch(() => ({ docs: [] })),
    listAgencyAccounts().catch(() => []),
  ]);

  const highFlow = await buildHighFlowSummary();
  const [metrics, usersPage, dueTodayPage, dueSoonPage, overduePage] = await Promise.all([
    buildAdminMetricsSnapshot(settings),
    listAdminUsersPage({ baseUrl, usageByUid: highFlow.usageByUid, ...usersQuery }),
    listAdminUsersPage({ baseUrl, usageByUid: highFlow.usageByUid, pageSize: 10, renewal: "today", sort: "expires_asc" }),
    listAdminUsersPage({ baseUrl, usageByUid: highFlow.usageByUid, pageSize: 10, renewal: "soon", sort: "expires_asc" }),
    listAdminUsersPage({ baseUrl, usageByUid: highFlow.usageByUid, pageSize: 10, renewal: "overdue", sort: "expires_asc" }),
  ]);
  const { usageByUid, ...highFlowPayload } = highFlow;
  ...
  return {
    ...
    usersPage,
    highFlow: highFlowPayload,
    agencies,
    renewalBuckets: {
      dueToday: dueTodayPage.items,
      dueSoon: dueSoonPage.items,
      overdue: overduePage.items,
    },
    overview: {
