...
const BARBER_TYPES = new Set(["barber_shop"]);
const BEAUTY_SERVICE_TYPES = new Set(["beauty_salon", "nails", "aesthetics"]);
const SPA_SERVICE_TYPES = new Set(["spa", "massage"]);
const WELLNESS_SERVICE_TYPES = new Set(["psychology", "therapy", "nutrition", "wellness_center"]);
const HEALTH_SERVICE_TYPES = new Set(["dental", "medical_office"]);
const REHAB_SERVICE_TYPES = new Set(["physical_therapy"]);
...
export function resolveCommerceExperience(input = {}, profile = {}) {
  ...
  if (businessCategory === "health_wellness") {
    if (BARBER_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
        subcategory: "barber",
        variant: "neutral",
        layout: activeMode === "mitienda" ? "store_grid" : "catalog_feed",
        theme: "barber_clean",
      };
    }

    if (BEAUTY_SERVICE_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
        subcategory: "beauty",
    ...
    if (SPA_SERVICE_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
        subcategory: "spa",
    ...
    if (WELLNESS_SERVICE_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
    ...
    if (HEALTH_SERVICE_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
        subcategory: "clinic",
    ...
    if (REHAB_SERVICE_TYPES.has(businessType)) {
      return {
        module: activeMode || "micatalogo",
        category: "health",
        subcategory: "rehab",
