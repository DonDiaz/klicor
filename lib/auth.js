...
function shouldHaveAdminAccess(decoded = {}) {
...
async function syncAdminClaims(decoded = {}) {
  ...
  const shouldBeAdmin = shouldHaveAdminAccess(decoded);
...
export async function verifyRequest(request) {
  ...
  const role = shouldHaveAdminAccess(decoded) ? "admin" : "user";
...
export function requireAdmin(context) {
  if (!shouldHaveAdminAccess(context?.decoded)) {
