...
export const ADMIN_PLAN_VALUES = [
  "trial",
  "annual",
  "institutional",
  "agency",
...
export const ADMIN_SETTINGS_DEFAULTS = {
  ...
  agencyAnnualPrice: 55000,
  partnerDefaultPrice: 55000,
};
...
export function normalizePlan(value = "") {
  const next = String(value || "").trim().toLowerCase();
  if (next === "anual") return "annual";
  if (next === "institucional") return "institutional";
  if (next === "agencia") return "agency";
  if (next === "cortesia") return "courtesy";
  return ADMIN_PLAN_VALUES.includes(next) ? next : "trial";
}
...
export function normalizeAdminSettings(input = {}) {
  const annualPrice = clampNumber(input.annualPrice, { min: 0, max: 1_000_000, fallback: ADMIN_SETTINGS_DEFAULTS.annualPrice });
  ...
  return {
    ...
    agencyAnnualPrice: clampNumber(input.agencyAnnualPrice, { min: 0, max: 1_000_000, fallback: annualPrice }),
    partnerDefaultPrice: clampNumber(input.partnerDefaultPrice, { min: 0, max: 1_000_000, fallback: annualPrice }),
  };
}
...
export function resolvePlanLabel(value = "") {
  return ADMIN_PLAN_OPTIONS.find((option) => option.value === normalizePlan(value))?.label || "Trial";
}
