...
export function buildLandingLayout(user = {}) {
  ...
  const profileLinks = mergeSystemProfileLinks(user.profileLinks || [], user);
  const sortedLinks = sortLinksByBusinessCategory(profileLinks, category);
  ...
  return {
    ...
    socialLinks,
    paymentMethods: Array.isArray(user.paymentMethods) ? user.paymentMethods.filter((method) => method?.entityId || method?.accountNumber || method?.brebKey) : [],
  };
}
