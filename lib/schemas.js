...
export const businessHoursSchema = z.any().transform((value) => normalizeBusinessHours(value || {}));
...
export const appearanceSchema = z.object({
  ...
  socialStyle: z.enum(["cards", "icons", "brand-circles"]).transform((value) => (value === "brand-circles" ? "icons" : value)),
...
export const profileSchema = z.object({
  ...
  profileLinks: z.array(profileLinkSchema),
...
export const adminUserAccessSchema = z.object({
  plan: z.enum(ADMIN_PLAN_VALUES),
  module: z.enum(["commerce", "booking"]).or(z.literal("")).default(""),
...
export const bookingConfigSchema = z.object({
  ...
  notifyBusinessOnRequest: z.boolean().default(true),
  notifyCustomerOnConfirmation: z.boolean().default(true),
  reminderEnabled: z.boolean().default(false),
  reminderMinutesBefore: z.coerce.number().int().min(15).max(1440).default(60),
  reactivationEnabled: z.boolean().default(false),
  reactivationDays: z.coerce.number().int().min(7).max(180).default(30),
...
export const bookingAppointmentCreateSchema = z.object({
  ...
  legalAcceptance: z.object({
    ...
    source: z.string().trim().max(80).optional().default("public_booking"),
    termsVersion: z.string().trim().max(80).optional().default(""),
    privacyVersion: z.string().trim().max(80).optional().default(""),
    acceptedAtClient: z.string().trim().max(80).optional().default(""),
  }).optional(),
});
...
export const bookingAppointmentScheduleSchema = bookingAppointmentCreateSchema.extend({
  id: z.string().trim().min(1),
});
